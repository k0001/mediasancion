...
@register.filter
def normalize_unicode(text, form, ensure_ascii=False):
    """
    Normalizes the given unicode string ``text`` using Unicode Normalization Form ``form``.

    If ``ensure_ascii`` is given, then non-ascii-supported characters are dropped from the resulting string.
    """
